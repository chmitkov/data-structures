...
public class BinaryTree {
    ...
    public List<Integer> topView() {
        Map<Integer, Tuple<Integer, Integer>> offsetToValueLevel = new TreeMap<>();
    ...
    private void traverseTree(BinaryTree binaryTree, int offset, int level, Map<Integer,
            Tuple<Integer, Integer>> offsetToValueLevel) {
        if (binaryTree == null) { return; }

        Tuple<Integer, Integer> currentValueLevel = offsetToValueLevel.get(offset);

        if (currentValueLevel == null || level < currentValueLevel.getValue()) {
            offsetToValueLevel.put(offset, new Tuple<>(binaryTree.value, level));
        }
